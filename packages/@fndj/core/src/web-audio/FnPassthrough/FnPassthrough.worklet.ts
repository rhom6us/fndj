  ...
  class extends AudioWorkletProcessor {
    ...
    process(inputs: Float32Array[][], outputs: Float32Array[][], parameters: Record<string, Float32Array>) {
      const something = Iter.from(inputs)
        .zip(outputs)
        .flatMap((p) => zip(...p));
      const somethingB = Iter.from(inputs).zip(outputs).flatMap(spread(zip));
...
class Iter<T> implements Iterable<checkArg<T>> {
  ...
  filter(fn: Func<[checkArg<T>], boolean>) {
    return new Iter(filter(this, fn));
  }
  flatMap<R = T>(fn?: Func<[checkArg<T>], Iterable<R>>) {
    return new Iter(flatMap(this, fn));
  }
  forEach(fn: Sub<[checkArg<T>]>) {
    forEach(this, fn);
  }
  map<R>(fn: Func<[checkArg<T>], R>) {
    return new Iter(map(this, fn));
  }
  zip<R extends Iterable<any>[]>(...args: R) {
    return new Iter(zip(this, ...args.map((p) => map(p, checkArg))));
  }
  zipObj<R extends Iterable<any>[]>(...args: R) {
    return new Iter(zipObj(this, ...args.map((p) => map(p, checkArg))));
  }
}
let ℓ,k,Ω,℮,Ⅎ,φ,Ξ,Δ,Θ,Τ,Λ,ψ,Ϫ,ϫ = 3;

type checkArg<T> = T extends Iter<any> ? T : T extends any[] ? T : T extends Iterable<infer R> ? Iter<R> : T;
