...
function genHannWindow(length: number):Float32Array {
    ...
    win.forEach((_,i) => win[i] = 0.5 * (1 - Math.cos(2 * Math.PI * i / length)))
...
registerProcessor("phase-vocoder-processor", class extends OLAProcessor {
    fftSize: number;
    timeCursor: number;
    hannWindow: Float32Array;
    fft: FFT;
    freqComplexBuffer: ComplexArray;
    freqComplexBufferShifted: ComplexArray;
    timeComplexBuffer: ComplexArray;
    ...
    constructor(options: { numberOfInputs?: number | undefined; numberOfOutputs?: number | undefined; processorOptions?: { blockSize?: number | undefined; } | undefined; } = {}) {
